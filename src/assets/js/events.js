...
window.addEventListener('load', () => {
    ...
    document.getElementById('on-light').addEventListener('click', (e) => {
        e.preventDefault();

        axios.post('https://api.thingspeak.com/update?api_key=B8B8E4FC1K541QZQ&field1=1')
            .then((res) => {
                if(res.data!=0)
                {
                console.log('Light On : ', res);
                document.querySelector('#off-light').attributes.removeNamedItem('hidden')
                document.getElementById('on-light').hidden = true;
                }
            }).catch((err) => {
                console.error(err);
            })
     ...
     document.getElementById('off-light').addEventListener('click', (e) => {
        e.preventDefault();

        axios.post('https://api.thingspeak.com/update?api_key=B8B8E4FC1K541QZQ&field1=0')
            .then((res) => {
                
                console.log('Light OFF: ', res);
                if(res.data!=0)
                {
                document.querySelector('#on-light').attributes.removeNamedItem('hidden')
                document.getElementById('off-light').hidden = true;
                }
            }).catch((err) => {
                console.error(err);
            })
    ...
    document.getElementById('off-fan').addEventListener('click', (e) => {
        e.preventDefault();

        axios.post('https://api.thingspeak.com/update?api_key=B8B8E4FC1K541QZQ&field2=0')
            .then((res) => {
                console.log('off Fan: ', res);
                // document.querySelector('#off-fan').attributes.removeNamedItem('hidden')
                document.getElementById('off-fan').hidden = true;
                  document.querySelector('#create-room').attributes.removeNamedItem('hidden')
            }).catch((err) => {
                console.error(err);
            })
     });
        ...
        document.getElementById('create-room').addEventListener('click', (e) => {
            ...
            axios.post('https://api.thingspeak.com/update?api_key=B8B8E4FC1K541QZQ&field2=1')
                .then((res) => {
                    console.log('On Fan: ', res);
                    document.querySelector('#off-fan').attributes.removeNamedItem('hidden')
                    document.getElementById('create-room').hidden = true;
                    // document.querySelector('#create-room').attributes.removeNamedItem('hidden')
                }).catch((err) => {
                    console.error(err);
                })
            ...
            //         axios.post('https://api.thingspeak.com/update?api_key=B8B8E4FC1K541QZQ&field2=0', user)
            //             .then((res) => {
